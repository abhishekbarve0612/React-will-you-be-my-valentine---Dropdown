...
function App() {
  ...
  const changeState = (e) => {
    if (e.target.value == -1) setSelectedState(null);
    else setSelectedState(parseInt(e.target.value));
  };

  const changeCity = (e) => {
    if (e.target.value == -1) setSelectedCity(null);
    else setSelectedCity(parseInt(e.target.value));
  };
  const changeLandmark = (e) => {
    if (e.target.value == -1) setSelectedLandmark(null);
    else setSelectedLandmark(parseInt(e.target.value));
  };
  // Do not alter/remove main div
  return (
    <div id="main" className="container m-auto mt-5">
      <div className="row">
        <div className="col-6">
          <StateDropdown states={states} changeState={changeState} />
